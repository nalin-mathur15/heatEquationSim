...
def heatEqn1D(u0, a, dx, dt, n, leftBound, rightBound):
    nx = len(u0)
    u = np.zeros((n, nx))
    u[0, :] = u0
...
def runSim():
    def start_simulation():
        ...
        leftBound = float(leftBound_var.get())
        rightBound = float(rightBound_var.get())
        
        u0 = initialCond(poly_str, nx, dx)
        u = heatEqn1D(u0, a, dx, dt, time_steps, leftBound, rightBound)
    ...
    def reset_values():
        ...
        leftBound_var.set(0.0)
        rightBound_var.set(0.0)
        for widget in plot_frame.winfo_children():
            widget.destroy()
    ...
    poly_entry = ttk.Entry(frame, width=25)
    poly_entry.insert(0, "x**2 - x")
    poly_entry.pack()
    
    a_var = tk.DoubleVar(value=0.1)
    ttk.Label(frame, text="Alpha").pack()
    ttk.Scale(frame, from_=0.01, to=1.0, variable=a_var, orient='horizontal').pack()
    ...
    leftBound_var = tk.DoubleVar(value=0.0)
    ttk.Label(frame, text="Left Boundary Condition").pack()
    leftBoundEntry = ttk.Entry(frame, textvariable=leftBound_var, width=25)
    leftBoundEntry.pack()
    
    rightBound_var = tk.DoubleVar(value=0.0)
    ttk.Label(frame, text="Right Boundary Condition").pack()
    rightBoundEntry = ttk.Entry(frame, textvariable=rightBound_var, width=25)
    rightBoundEntry.pack()
